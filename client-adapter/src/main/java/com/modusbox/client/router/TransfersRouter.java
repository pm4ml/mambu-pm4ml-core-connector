        ...
        from("direct:putTransfersByTransferId").routeId(ROUTE_ID).doTry()
            ...
            .to("bean:customJsonMessage?method=logJsonMessage('info', ${header.X-CorrelationId}, " +
                                                                "'Calling Mambu API, postTransaction, " +
                                                                   "POST https://{{easy.mambu.host}}/loans/${exchangeProperty.getLoanByIdResponse[0]?.get('id')}/repayment-transactions ', " +
                                                                "'Tracking the request', 'Track the response', 'Input Payload: ${body}')")
            .toD("https://{{easy.mambu.host}}/loans/${exchangeProperty.getLoanByIdResponse[0]?.get('id')}/repayment-transactions ")
            .to("bean:customJsonMessage?method=logJsonMessage('info', ${header.X-CorrelationId}, " +
