...
public class QuotesRouter extends RouteBuilder {
    ...
    public void configure() {
        ...
        from("direct:postQuoteRequests").routeId(ROUTE_ID).doTry()
			...
			.setHeader("mfiName", simple("{{easy.mfi-name}}"))
		...
		from("direct:getLoanScheduleById")
				...
				.to("bean:customJsonMessage?method=logJsonMessage('info', ${header.X-CorrelationId}, " +
						...
						"'Request sent to, GET https://{{easy.mambu.host}}/loans/${exchangeProperty.getLoanByIdResponse[0]?.get('id')}/schedule')")
				.toD("https://{{easy.mambu.host}}/loans/${exchangeProperty.getLoanByIdResponse[0]?.get('id')}/schedule")
